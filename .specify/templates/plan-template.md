...
- Confirm the feature preserves Dioxus 0.7 as the UI contract and introduces no removed APIs such as `cx`, `Scope`, or `use_state`.
- Confirm shared behavior remains in `packages/ui` and platform entrypoints remain thin unless the spec explicitly narrows the platform scope.
...
- Confirm the feature follows Rust formatting, naming, ownership, and error-handling standards.
- Confirm Dioxus code uses 0.7 APIs only: `Element`, `#[component]`, signals/resources/memos/context, `Router::<Route>`, and `asset!`.
